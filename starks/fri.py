...
def select_random(field: Field) -> FieldElement:
  result = hashlib.sha384(x.encode())
...
class AffineSubspaceFRI(object):
  ...
  def commit(self, f, security_factor:int = 40) -> List[bytes]:
    ...
    L_i0 = select_subspace(L_i, eta) # its dimention should be eta
    f_i = f

    for i in range(r):
      # This should be picked using a hash of some common data to apply the fiat-shamir heuristic
      x = str(R)+str(eta)+str(r)+str(k_0)+str(L_i)+str(L_i0)
      x_i = select_random(self.field, x)
      # Convention: L^i_0 is the space generated by the first eta basis
      q_i = construct_affine_vanishing_polynomial(L_i0)
      L_i_plus_1 = eval_on_subspace(q_i, L_i)
      if i < r-1:
        # define empty function
        f_i_plus_1 = {}
        for x in L_i:
          coset = [x + l_i for l_i in L_i]
          coset_eval = [f_i(x_c) for x_c in coset]
          interp = lagrange_interp(self.field, coset_eval)
          f_i_plus_1 = interp(x_i)
        # Does this lexical capture work?
        f_i = lambda z: f_i_plus_1[z]
        L_i = L_i_plus_1
      else:
        f_r = f_i
        L_r = L_i
        vals = [f_r(val) for val in self.affine_space]
        interp = lagrange_interp(self.field, vals)
        d = rho * len(L_r) - 1
        return interp.coefficients[:d+1]

  def query(self, f, l, L0, x, Pr):
    seed(1)
    # rate parameter
    R = -math.log(self.rho, 2)
    # localization parameter
    ...
    L = self.affine_space
    for j in range(l):
      random_n = randint(0, len(L))
      s_i_plus = []
      s_i_plus.append(L[random_n])
      S_i = []
      for i in range(r):
        q_i = construct_affine_vanishing_polynomial(L0[i])
        s_i_plus.append(q_i(s_i_plus[-1]))
        S_i.append([s_i_plus[i] + l_i for l_i in L0[i]])
      
      interp = []
      for i in range(r):
        coset_eval = [f(x_c) for x_c in S_i]
        interp.append(lagrange_interp(self.field, coset_eval))

      for i in range(r):
        if f[i](s_i_plus[i+1]) is not interp[i](x[i]):
          return False
    return True
