...
def select_subspace(space: AffineSpace) -> AffineSpace:
  field = space.field
  basis = space.basis
  sub_basis = basis[0:2]
...
class AffineSubspaceFRI(object):
  ...
  def commit(self, f, security_factor:int = 40) -> List[bytes]:
    ...
    L_i0 = select_subspace(L_i)
    f_i = f
    for i in range(r):
      # TODO: This should be picked using a hash of some common data to apply the fiat-shamir heuristic
      x_i = select_random(self.field)
