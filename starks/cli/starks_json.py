...
def exc_handler_to_dict(file_path: Union[str, None],
    ...
    output_json = {
        'compiler': f"starks-{starks.__version__}",
        'errors': [err_dict],
    }
    return output_json
...
def _standardize_path(path_str: str) -> str:
    path = Path("/__starks/" + path_str.lstrip('/')).resolve()
    try:
        path = path.relative_to("/__starks")
    except ValueError:
...
def get_input_dict_interfaces(input_dict: Dict) -> Dict:
    ...
    for path, value in input_dict.get('interfaces', {}).items():
        ...
        if key.endswith(".json"):
            ...
            interface = {'type': "starks", 'code': value['content']}
        else:
...
def compile_json(input_json: Union[Dict, str],
    ...
    try:
        ...
        return exc_handler(file_path, exc, "starks")
