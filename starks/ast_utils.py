import ast as python_ast
from typing import (
    Generator,
)

import asttokens

import starks.ast as starks_ast
from starks.exceptions import (
    CompilerPanic,
    ParserException,
    SyntaxException,
)
from starks.parser.parser_utils import (
    annotate_ast,
)
from starks.parser.pre_parser import (
    pre_parse,
)
from starks.utils import (
    iterable_cast,
)

DICT_AST_SKIPLIST = ('source_code', )

@iterable_cast(list)
def _build_starks_ast_list(source_code: str, node: list, source_id: int) -> Generator:
    for n in node:
        yield parse_python_ast(
            source_code=source_code,
            node=n,
            source_id=source_id,
        )


@iterable_cast(dict)
def _build_starks_ast_init_kwargs(
    source_code: str,
    node: python_ast.AST,
    starks_class: starks_ast.StarksNode,
    class_name: str,
    source_id: int,
) -> Generator:
    start = node.first_token.start if hasattr(node, 'first_token') else (None, None)  # type: ignore
    yield ('col_offset', start[1])
    yield ('lineno', start[0])
    yield ('node_id', node.node_id)  # type: ignore
    yield ('source_code', source_code)

    end = node.last_token.end if hasattr(node, 'last_token') else (None, None)  # type: ignore
    yield ('end_lineno', end[0])
    yield ('end_col_offset', end[1])
    if hasattr(node, 'last_token'):
        start_pos = node.first_token.startpos  # type: ignore
        end_pos = node.last_token.endpos  # type: ignore
        yield ('src', f"{start_pos}:{end_pos-start_pos}:{source_id}")

    if isinstance(node, python_ast.ClassDef):
        yield ('class_type', node.class_type)  # type: ignore

    for field_name in node._fields:
        val = getattr(node, field_name)
        if field_name in starks_class.ignored_fields:
            continue
        elif val and field_name in starks_class.only_empty_fields:
            raise SyntaxException(
                'Invalid Starks Syntax. '
                f'"{field_name}" is an unsupported attribute field '
                f'on Python AST "{class_name}" class.',
                val
            )
        else:
            yield (
                field_name,
                parse_python_ast(
                    source_code=source_code,
                    node=val,
                    source_id=source_id
                )
            )


def parse_python_ast(source_code: str,
                     node: python_ast.AST,
                     source_id: int = 0) -> starks_ast.StarksNode:
    if isinstance(node, list):
        return _build_starks_ast_list(source_code, node, source_id)
    elif isinstance(node, python_ast.AST):
        class_name = node.__class__.__name__
        if hasattr(starks_ast, class_name):
            starks_class = getattr(starks_ast, class_name)
            init_kwargs = _build_starks_ast_init_kwargs(
                source_code, node, starks_class, class_name, source_id
            )
            return starks_class(**init_kwargs)
        else:
            raise SyntaxException(
                f'Invalid syntax (unsupported "{class_name}" Python AST node).', node
            )
    else:
        return node


def parse_to_ast(source_code: str, source_id: int = 0) -> list:
    if '\x00' in source_code:
        raise ParserException('No null bytes (\\x00) allowed in the source code.')
    class_types, reformatted_code = pre_parse(source_code)
    py_ast = python_ast.parse(reformatted_code)
    annotate_ast(py_ast, source_code, class_types)
    asttokens.ASTTokens(source_code, tree=py_ast)
    # Convert to Starks AST.
    starks_ast = parse_python_ast(
        source_code=source_code,
        node=py_ast,
        source_id=source_id,
    )
    return starks_ast.body  # type: ignore


@iterable_cast(list)
def _ast_to_list(node: list) -> Generator:
    for x in node:
        yield ast_to_dict(x)


@iterable_cast(dict)
def _ast_to_dict(node: starks_ast.StarksNode) -> Generator:
    for f in node.get_slots():
        if f not in DICT_AST_SKIPLIST:
            yield (f, ast_to_dict(getattr(node, f, None)))
    yield ('ast_type', node.__class__.__name__)


def ast_to_dict(node: starks_ast.StarksNode) -> dict:
    if isinstance(node, starks_ast.StarksNode):
        return _ast_to_dict(node)
    elif isinstance(node, list):
        return _ast_to_list(node)
    elif node is None or isinstance(node, (str, int)):
        return node
    else:
        raise CompilerPanic('Unknown starks AST node provided.')


def dict_to_ast(ast_struct: dict) -> starks_ast.StarksNode:
    if isinstance(ast_struct, dict) and 'ast_type' in ast_struct:
        starks_class = getattr(starks_ast, ast_struct['ast_type'])
        klass = starks_class(**{
            k: dict_to_ast(v)
            for k, v in ast_struct.items()
            if k in starks_class.get_slots()
        })
        return klass
    elif isinstance(ast_struct, list):
        return [
            dict_to_ast(x)
            for x in ast_struct
        ]
    elif ast_struct is None or isinstance(ast_struct, (str, int)):
        return ast_struct
    else:
        raise CompilerPanic('Unknown ast_struct provided.')


def to_python_ast(starks_ast_node: starks_ast.StarksNode) -> python_ast.AST:
    if isinstance(starks_ast_node, list):
        return [
            to_python_ast(n)
            for n in starks_ast_node
        ]
    elif isinstance(starks_ast_node, starks_ast.StarksNode):
        class_name = starks_ast_node.__class__.__name__
        if hasattr(python_ast, class_name):
            py_klass = getattr(python_ast, class_name)
            return py_klass(**{
                k: to_python_ast(
                    getattr(starks_ast_node, k, None)
                )
                for k in starks_ast_node.get_slots()
            })
        else:
            raise CompilerPanic(f'Unknown starks AST class "{class_name}" provided.')
    else:
        return starks_ast_node


def ast_to_string(starks_ast_node: starks_ast.StarksNode) -> str:
    py_ast_node = to_python_ast(starks_ast_node)
    return python_ast.dump(
        python_ast.Module(
            body=py_ast_node
        )
    )
