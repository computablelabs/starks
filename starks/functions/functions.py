...
from starks.parser.expr import (
    Expr,
)
from starks.parser.keccak256_helper import (
    keccak256_helper,
)
...
from starks.types.convert import (
    convert,
)
...
# starks.parser.stmt so we can use _assert_reason).
@signature('bool')
def assert_modifiable(expr, args, kwargs, context):
    # cf. starks.parser.stmt.parse_assert
...
def get_create_forwarder_to_bytecode():
    from starks.compile_lll import (
        assembly_to_evm,
        num_to_bytearray
    )
    code_a = [
...
def sqrt(expr, args, kwargs, context):
    from starks.functions.utils import (
